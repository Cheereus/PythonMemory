...
def myTest_Kmeans_nclusters(data_train):
    """
    计算不同的k值时，SSE的大小变化
    """
    ...
    for num in nums:
        ...
        for index in range(len(data)):
            ...
            sse += distEclud(data_train_1[index, :], cluster_ceter_list[cluster_num])
        print("簇数是", num, "时； SSE是", sse)
        SSE.append(sse)
    return nums, SSE


print(filter_zscore_data)
nums, SSE = myTest_Kmeans_nclusters(filter_zscore_data)
